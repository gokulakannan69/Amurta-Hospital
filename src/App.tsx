...
import { AUTOMATION_CONFIG, DOCTOR_IMAGE_URL, CONTACT_CONFIG } from './constants/config';
                    ...
                    <div className="space-y-12 animate-in fade-in duration-500">
                        ...
                        <section className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm space-y-8">
                            <div className="text-center space-y-2">
                                <h3 className="text-2xl font-black text-blue-950 uppercase tracking-tight">{t.contactUs}</h3>
                                <div className="w-12 h-1 bg-gradient-to-r from-blue-500 to-emerald-500 rounded-full mx-auto"></div>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {/* Location Card */}
                                <a
                                    href={CONTACT_CONFIG.googleMap}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="bg-blue-50/50 p-6 rounded-[2rem] border border-blue-100/50 flex items-center gap-5 hover:bg-blue-50 hover:shadow-lg hover:scale-[1.02] transition-all group"
                                >
                                    <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center text-blue-600 shadow-sm group-hover:bg-blue-600 group-hover:text-white transition-colors">
                                        <MapPin className="w-8 h-8" />
                                    </div>
                                    <div className="flex-1">
                                        <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-1">{t.location}</p>
                                        <p className="font-bold text-blue-900 leading-tight">{t.getDirections}</p>
                                    </div>
                                ...
                                <a
                                    ...
                                    className="bg-emerald-50/50 p-6 rounded-[2rem] border border-emerald-100/50 flex items-center gap-5 hover:bg-emerald-50 hover:shadow-lg hover:scale-[1.02] transition-all group"
                                >
                                    <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center text-emerald-600 shadow-sm group-hover:bg-emerald-600 group-hover:text-white transition-colors">
                                        <Phone className="w-8 h-8" />
                                    </div>
                                    <div className="flex-1">
                                        <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-1">{t.phone}</p>
                                        <p className="font-bold text-blue-900 leading-tight mobile-numbers">{CONTACT_CONFIG.phone}</p>
                                    </div>
                    ...
                    activeTab === 'treatments' && (
                        <div className="space-y-8 animate-in fade-in duration-500">
                            <div className="text-center space-y-4 pb-8 border-b border-slate-100">
                                <h2 className="text-3xl font-black text-blue-950 uppercase tracking-tight">{t.intake}</h2>
                                <p className="text-sm md:text-lg text-emerald-700 font-bold italic">{t.section} 1 {t.of} 4</p>
                            </div>

                            <div className="max-w-xl mx-auto space-y-6">
                                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 text-center">{t.consultationFormat}</p>
                                <div className="grid gap-3">
                                    {SERVICE_MODES.map(m => (
                                        <button
                                            key={m.id}
                                            onClick={() => { setBookingData(prev => ({ ...prev, mode: lang === 'tm' ? m.tm : m.en })); setBookingStep(2); }}
